...
InstructBLIP（Towards General-purpose Vision-Language Models with Instruction Tuning, arXiv:2305.06500, Salesforce 2023）は、BLIP-2 を土台に「指示チューニング（instruction tuning）」を視覚言語へ持ち込んだモデルです。最大の貢献は、**指示文を LLM だけでなく Q-Former にも与える** という一点に集約されます。BLIP-2 を既習の読者にとって本稿の狙いは、この小さな配線変更が「タスクに応じて視覚特徴を作り変える」という能力をどう生み、なぜ未見タスクへのゼロショット汎化を押し上げるのかを腹落ちさせることです。最後に、発展研究 Adaptive Cluster-CLIP（ACC）の「クエリ非依存インデックス」と対比し、InstructBLIP の「クエリ依存」設計の射程と限界を整理します。
...
ここで発展研究 **Adaptive Cluster-CLIP（ACC）** との対比を置くと、InstructBLIP 設計の射程がくっきりします。InstructBLIP の本質は、**指示（=クエリ）に応じて視覚特徴を抽出し直す「クエリ依存」型**です。これは表現力の源泉である一方、運用上のコストを伴います。クエリが変われば Q-Former への指示入力も変わるため、**原理的には画像特徴をクエリごとに計算し直す**必要があります。一枚の画像に一つの指示、という対話用途では問題になりませんが、**一枚（あるいは大量）の画像に対して多数のクエリを当てる検索用途**では、クエリ数に比例して視覚側の再計算が積み上がります。

ACC はちょうど逆の設計思想を取ります。CLIP（Dense-CLIP）の局所特徴をクラスタリングして少数の集約ベクトルへ圧縮し、**クエリ非依存のインデックス**を一度だけ構築します。インデックスは特定のクエリに依存しないため、**多数のクエリで何度でも再利用**でき、クエリごとに視覚を抽出し直す必要がありません。検索のように「画像は固定・クエリが多数」という運用では、視覚側を作り直さない設計が効いてくる、という発想です。

<figure class="lec-fig"><svg viewBox="0 0 720 360" role="img" aria-label="InstructBLIPのクエリ依存の特徴抽出とACCのクエリ非依存インデックスを左右で対比した図" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif">
<rect x="15" y="46" width="330" height="300" rx="10" fill="#fafafa" stroke="#e4e4e7" stroke-width="2"/>
<text x="180" y="36" text-anchor="middle" font-size="13" font-weight="700" fill="#0e7490">InstructBLIP: クエリ依存</text>
<rect x="35" y="116" width="85" height="40" rx="8" fill="#e0e7ff" stroke="#4338ca" stroke-width="2"/>
<text x="77" y="141" text-anchor="middle" font-size="12" font-weight="700" fill="#3730a3">画像</text>
<rect x="35" y="186" width="85" height="40" rx="8" fill="#cffafe" stroke="#0e7490" stroke-width="2"/>
<text x="77" y="211" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">指示クエリ_i</text>
<rect x="150" y="148" width="110" height="56" rx="8" fill="#ecfeff" stroke="#0e7490" stroke-width="2"/>
<text x="205" y="173" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">Q-Former</text>
<text x="205" y="191" text-anchor="middle" font-size="11" fill="#0e7490">指示入力あり</text>
<rect x="278" y="153" width="58" height="46" rx="8" fill="#f4f4f5" stroke="#71717a" stroke-width="2"/>
<text x="307" y="180" text-anchor="middle" font-size="12" font-weight="700" fill="#18181b">特徴_i</text>
<line x1="120" y1="138" x2="146" y2="160" stroke="#71717a" stroke-width="2"/>
<polygon points="151,164 139,162 145,153" fill="#71717a"/>
<line x1="120" y1="204" x2="146" y2="186" stroke="#0e7490" stroke-width="2"/>
<polygon points="151,182 145,193 139,184" fill="#0e7490"/>
<line x1="260" y1="176" x2="274" y2="176" stroke="#71717a" stroke-width="2"/>
<polygon points="282,176 271,171 271,181" fill="#71717a"/>
<rect x="35" y="262" width="300" height="68" rx="8" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/>
<text x="185" y="288" text-anchor="middle" font-size="12" font-weight="700" fill="#b91c1c">クエリが変わるたび画像特徴を再計算</text>
<text x="185" y="312" text-anchor="middle" font-size="11" fill="#dc2626">多数クエリの検索運用では再計算が積み上がる</text>
<rect x="375" y="46" width="330" height="300" rx="10" fill="#fafafa" stroke="#e4e4e7" stroke-width="2"/>
<text x="540" y="36" text-anchor="middle" font-size="13" font-weight="700" fill="#16a34a">ACC: クエリ非依存</text>
<rect x="395" y="70" width="90" height="38" rx="8" fill="#e0e7ff" stroke="#4338ca" stroke-width="2"/>
<text x="440" y="94" text-anchor="middle" font-size="12" font-weight="700" fill="#3730a3">画像</text>
<rect x="380" y="126" width="170" height="40" rx="8" fill="#eef2ff" stroke="#6366f1" stroke-width="2"/>
<text x="465" y="151" text-anchor="middle" font-size="12" font-weight="700" fill="#4338ca">Dense-CLIP 局所特徴</text>
<rect x="380" y="188" width="200" height="48" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/>
<text x="480" y="210" text-anchor="middle" font-size="12" font-weight="700" fill="#15803d">集約インデックス</text>
<text x="480" y="227" text-anchor="middle" font-size="11" fill="#15803d">クラスタで圧縮・一度だけ構築</text>
<rect x="600" y="120" width="95" height="116" rx="8" fill="#cffafe" stroke="#0e7490" stroke-width="2"/>
<text x="647" y="150" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">指示1</text>
<text x="647" y="180" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">指示2</text>
<text x="647" y="210" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">指示…</text>
<line x1="440" y1="108" x2="440" y2="124" stroke="#71717a" stroke-width="2"/>
<polygon points="440,132 435,121 445,121" fill="#71717a"/>
<line x1="465" y1="166" x2="475" y2="186" stroke="#71717a" stroke-width="2"/>
<polygon points="479,192 467,189 474,181" fill="#71717a"/>
<line x1="598" y1="200" x2="582" y2="206" stroke="#16a34a" stroke-width="2"/>
<polygon points="575,209 585,201 587,212" fill="#16a34a"/>
<text x="592" y="190" text-anchor="middle" font-size="11" font-weight="700" fill="#16a34a">再利用</text>
<rect x="380" y="280" width="315" height="50" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/>
<text x="537" y="304" text-anchor="middle" font-size="12" font-weight="700" fill="#15803d">多数クエリでも索引は不変</text>
<text x="537" y="322" text-anchor="middle" font-size="11" fill="#15803d">抽出をやり直さず再利用できる</text>
</svg><figcaption>InstructBLIP（左）は<b>クエリ依存</b>で、指示が変わるたび視覚特徴を抽出し直す。ACC（右）は<b>クエリ非依存</b>のインデックスを一度構築し、多数クエリで再利用する。<b>表現力（指示適応）と再利用性（索引化）のトレードオフ</b>が対比の核心。</figcaption></figure>

この対比は優劣ではなく **トレードオフ**として読むべきです。InstructBLIP の「指示に応じて視覚を作り変える」柔軟さは、開放的な指示追従・対話・推論で強力です。一方、検索のように「同じ画像群を大量のクエリで何度も叩く」設定では、抽出を毎回やり直さない ACC のクエリ非依存索引が効率上有利になります。InstructBLIP が示した「視覚をクエリに合わせる」発想を、ACC は「視覚をクエリから切り離して索引化する」方向へ反転させた、と位置づけると両者の関係が見通せます。
...
- InstructBLIP の「クエリ依存」設計が苦手とする運用はどんな場面か。ACC の「クエリ非依存インデックス」はその弱点をどう回避し、代わりに何を諦めているか。
