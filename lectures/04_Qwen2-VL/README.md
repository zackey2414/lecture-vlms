...
Qwen-VL までの VLM は、入力画像を 224×224 のような固定解像度へリサイズしてから視覚トークン列に変換していました。Qwen2-VL（*Enhancing Vision-Language Model's Perception of the World at Any Resolution*, arXiv:2409.12191）は、この「あらかじめ決め打ちした解像度」という前提を捨て、**任意解像度の画像を可変個の視覚トークンへ動的に変換する Naive Dynamic Resolution** と、**位置を時間・高さ・幅の3成分へ分解する M-RoPE** の2点で知覚を拡張します。本ページでは、この2つの仕組みがなぜ自然に「画像／複数画像／動画を1モデルで」扱う基盤になるのかを腹落ちさせ、発展研究 ACC（物体中心の Open-Vocabulary 動画フレーム検索）との関係まで見通します。
...
モデル構成は次の通りです。視覚エンコーダは全サイズ共通で、総パラメータ数の通称（2B/7B/72B）と内部の LLM 規模が一致しない点に注意してください。
...
発展研究 **ACC（Adaptive Cluster-CLIP）** は、物体中心の Open-Vocabulary 動画フレーム検索を高速化する手法です。ACC は CLIP（Dense-CLIP）の局所特徴をクラスタリングして少数の集約ベクトルへ圧縮し、この**クエリ非依存のインデックス**を多数のクエリで再利用します。さらに MI-AFS 由来の変化スコアで時間的冗長性を測り、フレームごとの集約ベクトル数を適応的に配分します（GradAlloc）。動きの少ない区間は少ない集約ベクトルで、変化の大きい区間は多めに割り当てる、という発想です。

ここで Qwen2-VL の立ち位置が見えてきます。M-RoPE の時間軸はフレーム列を扱う**動画対応の基盤**ですが、MLLM を検索に使うと、**クエリごとに全フレームを再処理**する必要があり、クエリ数が増えるほど検索レイテンシが支配的になります。ACC 論文はこの点で MLLM（Qwen 系）と対比し、ACC はクエリ非依存インデックスの再利用によって**約 130 倍高速**だと報告しています。つまり「動画を時間的冗長性を含む系列として理解する」という同じ問題に対し、Qwen2-VL は M-RoPE で**汎用的な理解の基盤**を、ACC は**検索用途に特化した軽量インデックス**を与える、という相補的な関係にあります。

<figure class="lec-fig"><svg viewBox="0 0 720 260" role="img" aria-label="MLLMによる動画検索とACCの比較図。MLLMはクエリごとに全フレームを再処理して重い一方、ACCはクエリ非依存インデックスを再利用して高速であることを示す" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif">
  <text x="180" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="#dc2626">MLLM（Qwen 系）で検索</text>
  <rect x="40" y="50" width="280" height="170" rx="6" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/>
  <text x="70" y="80" font-size="12" font-weight="700" fill="#dc2626">クエリ毎に全フレーム再処理</text>
  <rect x="70" y="95" width="20" height="20" fill="#fff" stroke="#dc2626" stroke-width="2"/>
  <rect x="96" y="95" width="20" height="20" fill="#fff" stroke="#dc2626" stroke-width="2"/>
  <rect x="122" y="95" width="20" height="20" fill="#fff" stroke="#dc2626" stroke-width="2"/>
  <rect x="148" y="95" width="20" height="20" fill="#fff" stroke="#dc2626" stroke-width="2"/>
  <rect x="174" y="95" width="20" height="20" fill="#fff" stroke="#dc2626" stroke-width="2"/>
  <text x="70" y="150" font-size="12" fill="#dc2626">クエリ1 → 再処理</text>
  <text x="70" y="172" font-size="12" fill="#dc2626">クエリ2 → 再処理</text>
  <text x="70" y="194" font-size="12" fill="#dc2626">クエリ3 → 再処理 …</text>
  <text x="180" y="214" text-anchor="middle" font-size="12" font-weight="700" fill="#dc2626">レイテンシが支配的</text>
  <text x="540" y="32" text-anchor="middle" font-size="13" font-weight="700" fill="#16a34a">ACC（Cluster-CLIP）</text>
  <rect x="400" y="50" width="280" height="170" rx="6" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/>
  <text x="430" y="80" font-size="12" font-weight="700" fill="#16a34a">クエリ非依存インデックスを一度構築</text>
  <rect x="430" y="95" width="56" height="22" fill="#fff" stroke="#16a34a" stroke-width="2"/>
  <text x="458" y="110" text-anchor="middle" font-size="11" fill="#16a34a">集約ベクトル</text>
  <text x="500" y="110" font-size="11" fill="#16a34a">（クラスタ圧縮）</text>
  <text x="430" y="150" font-size="12" fill="#16a34a">クエリ1 ↘</text>
  <text x="430" y="172" font-size="12" fill="#16a34a">クエリ2 → 同じ index を再利用</text>
  <text x="430" y="194" font-size="12" fill="#16a34a">クエリ3 ↗</text>
  <text x="540" y="214" text-anchor="middle" font-size="12" font-weight="700" fill="#16a34a">約 130× 高速（論文比）</text>
</svg><figcaption>動画検索での対比。<b>MLLM はクエリ毎に全フレームを再処理</b>して重い一方、<b>ACC はクエリ非依存インデックスを再利用</b>して高速です。Qwen2-VL の M-RoPE 時間軸は理解の基盤ですが、検索用途では重い、という位置づけです。</figcaption></figure>
...
- 「動画を理解する」Qwen2-VL と「動画から検索する」ACC は、なぜ別々の最適化を必要とするのか。検索レイテンシが支配的になる条件は何か。
