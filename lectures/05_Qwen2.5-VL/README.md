...
Qwen2.5-VL（arXiv:2502.13923）は、Qwen-VL / Qwen2-VL の系譜を継ぎつつ、ViT への **window attention** 導入、M-RoPE 時間成分の **絶対時間（秒）** への対応付け、**長尺動画**理解、そして HTML 形式での**文書・レイアウト解析**を一段押し進めたフラッグシップである。読者はすでに LLaVA / Qwen-VL / Qwen2-VL を踏まえているので、本章では「何が新しく、なぜ効くのか」を中心に腹落ちさせる。とりわけこのモデルは、あなたの ACC（Adaptive Cluster-CLIP）研究で **MLLM 検索ベースライン**として比較対象になっており、後半では「MLLM をそのまま検索に使うことの重さ」を定量的に体感する。
...
ここが本章の核心だ。あなたの **ACC（Adaptive Cluster-CLIP）**は、物体中心 Open-Vocabulary 動画フレーム検索（OCOV-VFR）のための手法で、Dense-CLIP（ResNet-50）の局所特徴をクラスタリングして少数の集約ベクトルへ圧縮し、その**クエリ非依存インデックス**を多数のクエリで再利用する。MI-AFS 由来の変化スコアで時間区間を分割し、GradAlloc で時間的冗長性に応じて各フレームの集約ベクトル数を適応配分する。Jetson AGX Orin 上で **34.72 FPS（動画ソース FPS の 1.389 倍）**でインデックスを構築できる。

ACC 論文の表 7 では、Qwen2.5-VL **3B / 7B** が「全フレームを逐次処理する MLLM 検索ベースライン」として並べられている。ここで Qwen2.5-VL の強みが、検索という用途では**そのまま弱点**に反転する。MLLM は本質的に **クエリごとに全フレームを再処理**しなければならず、検索レイテンシが全コストを支配してしまうのだ。

<figure class="lec-fig"><svg viewBox="0 0 720 320" role="img" aria-label="検索レイテンシの対比。Qwen2.5-VL 3Bはクエリ毎に全フレーム再処理で約455102秒、ACCはインデックス再利用で約130倍高速" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif">
<text x="360" y="34" text-anchor="middle" font-size="14" font-weight="700" fill="#18181b">検索レイテンシ（Search）の対比</text>
<rect x="40" y="60" width="300" height="120" rx="10" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/>
<text x="190" y="86" text-anchor="middle" font-size="13" font-weight="700" fill="#dc2626">Qwen2.5-VL 3B をそのまま検索</text>
<text x="190" y="112" text-anchor="middle" font-size="12" fill="#dc2626">クエリ毎に全フレームを再処理</text>
<rect x="60" y="126" width="260" height="26" rx="4" fill="#dc2626"/>
<text x="190" y="144" text-anchor="middle" font-size="12" font-weight="700" fill="#ffffff">Search ≈ 455,102 秒</text>
<text x="190" y="170" text-anchor="middle" font-size="11" fill="#71717a">クエリ非依存の再利用ができない</text>
<rect x="380" y="60" width="300" height="120" rx="10" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/>
<text x="530" y="86" text-anchor="middle" font-size="13" font-weight="700" fill="#15803d">ACC（インデックス再利用）</text>
<text x="530" y="112" text-anchor="middle" font-size="12" fill="#15803d">クエリ非依存インデックスを多数クエリで共有</text>
<rect x="400" y="126" width="20" height="26" rx="4" fill="#16a34a"/>
<text x="530" y="144" text-anchor="middle" font-size="12" font-weight="700" fill="#15803d">約130倍高速</text>
<text x="530" y="170" text-anchor="middle" font-size="11" fill="#71717a">構築も対OWLv2-baseで8.8倍高速</text>
<rect x="40" y="206" width="640" height="92" rx="10" fill="#eef2ff" stroke="#4338ca" stroke-width="2"/>
<text x="360" y="232" text-anchor="middle" font-size="12" font-weight="700" fill="#4338ca">違いの本質：前処理コストをクエリ間で「償却」できるか</text>
<text x="360" y="256" text-anchor="middle" font-size="11" fill="#18181b">MLLM＝クエリ依存（毎回全フレーム） ／ ACC＝クエリ非依存インデックス（一度構築→再利用）</text>
<text x="360" y="278" text-anchor="middle" font-size="11" fill="#71717a">精度面でも Tight mAP@50 で対Cluster-CLIP +0.111</text>
</svg><figcaption>MLLM をそのまま検索に使うと <b>クエリごとに全フレーム再処理</b>が必要で、Qwen2.5-VL 3B の Search は <b>約 455,102 秒</b>に達する。ACC は <b>クエリ非依存インデックスの再利用</b>で <b>約 130 倍高速</b>、構築も対 OWLv2-base で <b>8.8 倍高速</b>、精度も Tight mAP@50 で <b>+0.111</b>（対 Cluster-CLIP）。</figcaption></figure>

この対比から読み取るべきは「MLLM が弱い」ではなく、**MLLM の使いどころ**だ。Qwen2.5-VL は秒単位の時間 grounding・絶対座標 grounding・構造化された根拠出力という強力な能力を持つ。しかしそれは**クエリ依存・全フレーム処理**を前提とした能力であり、何千ものクエリを大規模動画コーパスに投げる検索シナリオでは、前処理をクエリ間で償却できないために破綻する。ACC はここを「**クエリ非依存の前処理（クラスタ集約ベクトルのインデックス化）**」で切り分けた。

発展研究の出発点として、では MLLM をどこに組み込むか。素直な候補は次の三つだ。

- **クエリ非依存の前処理に限定して使う**：全フレーム逐次処理ではなく、ACC が選んだ**候補フレーム／クラスタ領域**に対してのみ、クエリ非依存に MLLM を走らせて意味豊かな記述子を作る。
- **根拠説明（grounding/rationale）**：最終候補フレームに対してだけ MLLM を呼び、なぜ一致したか（bbox・秒）を構造化出力させる。コストはトップ数件に限定される。
- **再ランキング（re-ranking）**：軽量な ACC インデックスで広く絞り、上位だけを Qwen2.5-VL の精緻な視覚言語理解で並べ替える二段構え。

いずれも共通する設計原理は「**重い MLLM 処理をクエリ間・候補件数で償却できる位置に押し込む**」こと。表 7 のレイテンシ差は、その押し込み先を間違えるとどれだけ高くつくかを定量的に突きつけている。
...
4. ACC 表 7 で Qwen2.5-VL 3B の Search が約 455,102 秒に達する根本原因は何か。これは「モデルが遅い」のか「使い方が検索に向かない」のか。
5. もし自分の検索パイプラインに Qwen2.5-VL を組み込むなら、どの段（前処理・再ランキング・根拠説明）に置けば、表 7 のレイテンシ破綻を避けつつ強みを活かせるか。
