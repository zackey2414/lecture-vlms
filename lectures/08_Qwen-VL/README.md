...
Qwen-VL（arXiv:2308.12966, Alibaba Group）は、LLM基盤に Qwen-7B を据えた汎用ビジョン言語モデル（LVLM）です。LLaVA系が「ViT特徴を MLP で言語空間へ**射影**してそのまま流し込む」のに対し、Qwen-VL は**学習可能な256個のクエリによる単層 cross-attention** で視覚特徴を 256 トークンに**圧縮**するコネクタ（Position-aware Vision-Language Adapter）を採用します。さらに、低解像度→高解像度→対話 の**3段階学習**と、bbox を座標テキストトークンで表す **grounding / OCR** を一つのモデルに統合した点が特徴です。本ページでは、これらを LLaVA系と対比しながら、最終セクションで発展研究 Adaptive Cluster-CLIP (ACC) との設計上の接点・相違を整理します。

---

## 全体像（まず一枚で）

Qwen-VL は 3 つの部品からなります。視覚エンコーダ（OpenCLIP の **ViT-bigG/14** 由来の ViT）、コネクタ（**Position-aware Vision-Language Adapter**）、そして言語モデル（**Qwen-7B**）です。画像は固定解像度（最終的に 448×448）にリサイズされ、stride 14 のパッチに分割されて多数の ViT 特徴になります。Adapter はこの可変長の特徴列を**固定長 256 トークン**へ圧縮し、`<img>` … `</img>` の特殊トークンで囲んでテキストトークン列に差し込み、Qwen-7B が自己回帰でテキスト（説明・回答・座標）を生成します。
...
- **Stage 1：事前学習（LLM 凍結・低解像度 224）。** Web 由来の弱教師 image-text ペア（クリーニング後 ≈1.4B、英語約77%・中国語約23%）で、**ViT と Adapter のみ**を学習します。LLM（Qwen-7B）は凍結。目的は視覚側を言語空間へ大まかに整列させること。入力は 224×224。
- **Stage 2：マルチタスク事前学習（全解凍・高解像度 448）。** 解像度を 224 → **448×448** に引き上げ、**モデル全体を解凍**して 7 種のタスクを同時学習します：captioning / VQA / grounding / referring grounding / grounded captioning / **OCR・text-reading** / pure-text autoregression。interleaved（画像とテキストが交互に並ぶ）系列も使い、grounding・OCR といった**細粒度能力**をここで獲得します。
...
| --- | --- | --- | --- | --- |
| Stage 1 事前学習 | 224×224 | LLM | ViT + Adapter | 弱教師 image-text、視覚↔言語の整列 |
| Stage 2 マルチタスク | 448×448 | なし（全解凍） | 全体 | captioning/VQA/grounding/OCR/referring |
| Stage 3 SFT | 448×448 | ViT | LLM + Adapter | 指示・対話、Qwen-VL-Chat 化 |
...
発展研究 **Adaptive Cluster-CLIP（ACC）** は、物体中心の **Open-Vocabulary 動画フレーム検索**を狙う手法です。CLIP（Dense-CLIP）の局所特徴をクラスタリングして少数の集約ベクトルに圧縮し、**クエリ非依存（query-agnostic）のインデックス**として事前構築します。一度作ったこのインデックスは、多数の自然言語クエリで**繰り返し再利用**できるのが要点で、論文では分離型 OVOD や MLLM と対比して「クエリ非依存に作ったインデックスの再利用」が効率の鍵だと論じます。

ここで Qwen-VL を置くと、設計の接点と差がはっきりします。

- **接点：** Qwen-VL は grounding を備え、「画像中のどこに何があるか」を座標として出せます。物体中心に画像内容を扱うという発想は、ACC の物体中心検索と方向性を共有します。
- **相違（ACC が突く点）：** Qwen-VL は本質的に**クエリ（プロンプト）ごとに**画像を LLM で処理して回答や座標を生成します。つまり「テキストクエリ → 画像処理 → 出力」が毎回走り、**クエリ非依存に固定したインデックスを先に作って使い回す**という形に向きません。フレーム×クエリの組合せが増えるほど、生成的処理を都度回すコストが効いてきます。

<figure class="lec-fig"><svg viewBox="0 0 760 250" role="img" aria-label="Qwen-VLはクエリごとに画像をLLM処理するのに対し、ACCはクエリ非依存インデックスを事前構築して多数クエリで再利用することを対比した図" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif">
<text x="20" y="34" font-size="14" font-weight="700" fill="#991b1b">Qwen-VL: クエリごとに毎回 画像をLLM処理</text>
<rect x="30" y="48" width="110" height="44" rx="8" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/><text x="85" y="75" text-anchor="middle" font-size="12" font-weight="700" fill="#991b1b">クエリ＋画像</text>
<line x1="142" y1="70" x2="178" y2="70" stroke="#71717a" stroke-width="2"/><polygon points="185,70 175,65 175,75" fill="#71717a"/>
<rect x="187" y="48" width="120" height="44" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/><text x="247" y="75" text-anchor="middle" font-size="12" font-weight="700" fill="#166534">LLM処理（毎回）</text>
<line x1="309" y1="70" x2="345" y2="70" stroke="#71717a" stroke-width="2"/><polygon points="352,70 342,65 342,75" fill="#71717a"/>
<rect x="354" y="48" width="100" height="44" rx="8" fill="#f4f4f5" stroke="#71717a" stroke-width="2"/><text x="404" y="75" text-anchor="middle" font-size="12" font-weight="700" fill="#3f3f46">回答/座標</text>
<text x="475" y="74" font-size="11" font-weight="700" fill="#71717a">→ インデックス再利用に不向き</text>
<line x1="20" y1="118" x2="740" y2="118" stroke="#e4e4e7" stroke-width="2"/>
<text x="20" y="150" font-size="14" font-weight="700" fill="#155e75">ACC: クエリ非依存インデックスを事前構築 → 多数クエリで再利用</text>
<rect x="30" y="164" width="120" height="44" rx="8" fill="#cffafe" stroke="#0e7490" stroke-width="2"/><text x="90" y="183" text-anchor="middle" font-size="11" font-weight="700" fill="#155e75">CLIP局所特徴</text><text x="90" y="199" text-anchor="middle" font-size="11" font-weight="700" fill="#155e75">→クラスタ圧縮</text>
<line x1="152" y1="186" x2="188" y2="186" stroke="#71717a" stroke-width="2"/><polygon points="195,186 185,181 185,191" fill="#71717a"/>
<rect x="197" y="164" width="150" height="44" rx="8" fill="#e0e7ff" stroke="#4338ca" stroke-width="2"/><text x="272" y="183" text-anchor="middle" font-size="11" font-weight="700" fill="#3730a3">クエリ非依存</text><text x="272" y="199" text-anchor="middle" font-size="11" font-weight="700" fill="#3730a3">インデックス（1回）</text>
<line x1="349" y1="186" x2="385" y2="186" stroke="#71717a" stroke-width="2"/><polygon points="392,186 382,181 382,191" fill="#71717a"/>
<rect x="394" y="156" width="120" height="60" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/><text x="454" y="180" text-anchor="middle" font-size="11" font-weight="700" fill="#166534">クエリ1..N</text><text x="454" y="198" text-anchor="middle" font-size="11" font-weight="700" fill="#166534">で高速検索</text>
<text x="535" y="190" font-size="11" font-weight="700" fill="#71717a">→ 多数クエリで償却</text>
</svg><figcaption>Qwen-VL は<b>クエリ依存の生成処理</b>を毎回実行。ACC は<b>クエリ非依存インデックスを一度だけ構築</b>し、多数クエリでコストを償却します。これが ACC の対比点です。</figcaption></figure>

要するに、Qwen-VL の grounding は「画像1枚に対し正確に局在化する」生成能力に強みがあり、ACC は「大量フレーム×大量クエリを安く捌く」検索効率に強みがあります。ACC が分離型・インデックス再利用を選ぶ動機は、まさに MLLM 系（Qwen-VL を含む）が苦手とする**クエリ非依存の事前計算**の欠落を突くところにあります。
...
4. ACC の「クエリ非依存インデックス再利用」と、Qwen-VL の「クエリごとの生成処理」は、動画フレーム検索においてどのトレードオフを生むか。Qwen-VL の grounding をそのまま大規模検索に使うと何がボトルネックになるか。
