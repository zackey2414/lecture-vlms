...
| 視覚位置符号化 | 通常の +1 刻み（V2PE で δ=1 に相当） | **V2PE（δ&lt;1）** で可変解像度・長文脈に対応 |
...
学習の定式化は素直な左から右への自己回帰ですが、**損失計算をテキストトークンに限定**するのが要点です。視覚トークンは予測対象ではなく「条件付けの文脈」として働き、勾配はテキスト予測に有用な形でマルチモーダル情報を埋め込ませます。トークン長による勾配バイアスを抑えるため、重み付けには **square averaging**（`w_i ∝ 1/l^0.5`）を採用します。

さらに **Joint Parameter Optimization** として、整列段で一部を凍結する従来流儀と異なり、**ViT・MLP・LLM の全層を同時に最適化**します。これにより、テキスト表現と視覚特徴が「同じ釜」で協調して形成され、別途のブリッジモジュールや後段のモデル間整列を要しません。データ混合は経験的に **言語:多モーダル ≈ 1:3** が最良で、総計はおよそ 200B トークン（言語 50B＋多モーダル 150B）でした。
...
ネイティブ事前学習の核心（PDF 2.2 節）
  ...
  データ比 : 言語 : 多モーダル ≈ 1 : 3（計 約200B token）
```
...
ネイティブ/可変解像度の多モーダル入力では、高解像画像や長尺動画で視覚トークン数が膨れ上がり、通常の「全トークン +1 刻み」では位置インデックスが急速に伸びて、LLM の有効文脈窓を食い潰します。InternVL3 は **V2PE（Variable Visual Position Encoding）** を採用し、視覚トークンに対しては **1 より小さい増分 δ** で位置を進めます。
...
δ ∈ { 1, 1/2, 1/4, 1/8, 1/16, 1/32, 1/64, 1/128, 1/256 }
  学習時 : 画像ごとに δ をランダム選択（画像内では一定に保つ）
  推論時 : 入力系列長に応じて δ を柔軟に選び、位置を有効文脈内に収める
  δ = 1 のとき InternVL2.5 と同じ通常の位置符号化に戻る
```
...
アブレーション（PDF Table 12, InternVL3-8B）では、V2PE の導入が多くの指標で性能を押し上げ、しかも短文脈中心のタスクでも**中程度に小さい δ（1/4 付近）**が好成績でした。論文本文では公平比較のため Table 12 以外の結果は δ=1 に固定しています。視覚トークンの位置を「テキストと同格に 1 ずつ進める」必然性はなく、**モダリティごとに位置の進み方を変えてよい**という発想が V2PE の肝です。
...
`L = w_p·L_p + w_q·L_q + w_g·L_g`。MPO 用の選好ペアは MMPR v1.2 から構築し約 300K サンプル、SFT データの部分集合であるため、**改善はアルゴリズム由来**（データ増ではない）と論文は強調します。Table 13 では MPO により 78B/38B でそれぞれ **+4.1 / +4.5 ポイント**の推論性能向上が報告されています。
...
本講義の発展研究 **ACC（Adaptive Cluster-CLIP）** は、物体中心の動画フレーム検索を扱います。CLIP（ResNet-50 ベースの Dense-CLIP）の局所特徴をクラスタリングして少数の集約ベクトルへ圧縮し、**クエリ非依存のインデックス**を一度作れば多数のクエリで再利用できる、という設計です。生成型 MLLM はクエリごとに全フレームを再処理する必要があり検索レイテンシが支配的になるのに対し、ACC はインデックス再利用で**約 130 倍**高速とされます。

InternVL3 はこの図式の中で「**最新・高精度だが、検索インデックスとしては重い**」MLLM の代表例です。72.2 MMMU・906 OCRBench といった精度は魅力的でも、検索のたびに全フレームへ重い前向き計算を走らせれば、クエリ非依存インデックスの再利用という ACC の本質的な強みは得られません。

<figure class="lec-fig"><svg viewBox="0 0 720 300" role="img" aria-label="生成型MLLMはクエリごとに全フレームを再処理するためレイテンシが支配的になるのに対し、ACCはクエリ非依存インデックスを一度構築して多数クエリで再利用するため約130倍高速である対比図" font-family="ui-sans-serif, system-ui, 'Noto Sans JP', sans-serif"><text x="30" y="36" font-size="13" font-weight="700" fill="#dc2626">生成型 MLLM（InternVL3 等）を検索に使うと</text><rect x="30" y="52" width="120" height="48" rx="8" fill="#ecfeff" stroke="#0e7490" stroke-width="2"/><text x="90" y="81" text-anchor="middle" font-size="12" font-weight="700" fill="#0e7490">クエリ q</text><line x1="150" y1="76" x2="178" y2="76" stroke="#71717a" stroke-width="2"/><polygon points="186,76 175,71 175,81" fill="#71717a"/><rect x="188" y="52" width="240" height="48" rx="8" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/><text x="308" y="74" text-anchor="middle" font-size="11" font-weight="700" fill="#dc2626">全フレームを毎回 再処理</text><text x="308" y="91" text-anchor="middle" font-size="10" fill="#dc2626">クエリごとに重い前向き計算</text><line x1="428" y1="76" x2="456" y2="76" stroke="#71717a" stroke-width="2"/><polygon points="464,76 453,71 453,81" fill="#71717a"/><rect x="466" y="52" width="120" height="48" rx="8" fill="#fee2e2" stroke="#dc2626" stroke-width="2"/><text x="526" y="81" text-anchor="middle" font-size="12" font-weight="700" fill="#dc2626">高レイテンシ</text><text x="30" y="160" font-size="13" font-weight="700" fill="#15803d">ACC：クエリ非依存インデックスを再利用</text><rect x="30" y="176" width="200" height="48" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/><text x="130" y="198" text-anchor="middle" font-size="11" font-weight="700" fill="#15803d">局所特徴をクラスタ集約</text><text x="130" y="215" text-anchor="middle" font-size="10" fill="#15803d">索引を一度だけ構築</text><line x1="230" y1="200" x2="258" y2="200" stroke="#71717a" stroke-width="2"/><polygon points="266,200 255,195 255,205" fill="#71717a"/><rect x="268" y="176" width="170" height="48" rx="8" fill="#cffafe" stroke="#0e7490" stroke-width="2"/><text x="353" y="205" text-anchor="middle" font-size="11" font-weight="700" fill="#0e7490">多数クエリで再利用</text><line x1="438" y1="200" x2="466" y2="200" stroke="#71717a" stroke-width="2"/><polygon points="474,200 463,195 463,205" fill="#71717a"/><rect x="476" y="176" width="180" height="48" rx="8" fill="#dcfce7" stroke="#16a34a" stroke-width="2"/><text x="566" y="198" text-anchor="middle" font-size="12" font-weight="700" fill="#15803d">約 130× 高速</text><text x="566" y="215" text-anchor="middle" font-size="10" fill="#15803d">検索レイテンシを償却</text><text x="360" y="262" text-anchor="middle" font-size="11" fill="#71717a">→ MLLM は「索引そのもの」ではなく、前処理・根拠説明・再ランキングに置く</text></svg><figcaption>MLLM は<b>クエリごと全フレーム再処理</b>でレイテンシ支配。ACC は<b>クエリ非依存インデックスの再利用</b>で償却します。</figcaption></figure>

| 観点 | InternVL3（生成型 MLLM） | ACC（Adaptive Cluster-CLIP） |
|---|---|---|
| 索引の依存性 | クエリ依存（毎回フレームを再処理） | **クエリ非依存**（一度構築して再利用） |
| 多クエリ時の計算 | クエリ数に比例して増大 | 索引構築コストを償却 |
| 検索レイテンシ | 支配的になりやすい | ACC で約 **130×** 高速 |
| 精度 | 最新・高精度 | 検索特化で軽量・実用的 |

発展研究での落とし所は、**最新 MLLM を「検索の主役」にせず、適所に組み込む**ことです。例えば、(1) オフラインの**クエリ非依存前処理**（候補フレームへの密なキャプション付与や物体タグ生成）、(2) 上位候補に対する**根拠説明・再ランキング**、(3) 難ケースだけに MLLM 推論を回す段階的パイプライン、といった形なら、InternVL3 の高精度を、ACC の索引再利用がもたらす低レイテンシと両立させられます。

## まとめと、読解後に答えたい問い

- InternVL3 の核心は **native multimodal pretraining**：テキスト専用 LLM を後付け適応するのではなく、**1 段で視覚＋言語を同時に**学び、ViT・MLP・LLM の全パラメータを同時最適化する（損失はテキストトークンのみ、言語:多モーダル ≈ 1:3、約 200B トークン）。
- **V2PE** は視覚トークンの位置を δ&lt;1 で緩やかに進め、可変解像度・長文脈を有効文脈窓に収める。δ=1 で従来法に一致する後方互換設計。
...
- ACC との対比では、InternVL3 は高精度だが検索索引としては重い。**クエリ非依存索引の再利用**という ACC の強みと噛み合わせるには、MLLM を前処理・根拠説明・再ランキングへ配置するのが筋。

読解後に答えたい問い:

1. 「損失をテキストトークンのみに限定する」設計は、視覚特徴の質にどう影響するか。視覚側にも損失を置く設計（対照学習やマスク再構成）と比べた利点・欠点は何か。
2. V2PE の δ をなぜ画像内では一定に保つのか。δ を可変にする/位置を非線形に進める拡張は、相対位置関係をどこまで壊さずに長文脈を稼げるか。
...
5. ネイティブ事前学習で得た「同時学習された視覚特徴」を、ACC のようなクエリ非依存索引にそのまま転用できるか。検索向けに蒸留・凍結するとしたら、どの層をどう取り出すべきか。
